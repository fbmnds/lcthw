...
void*
You_move(void *self, Direction direction) {
    You            *you = self;
    Room           *next = NULL;
    ...
    case NORTH:
        if (you->location->north)
            next = you->location->north;
        printf("You go north.\n");
        break;
    case EAST:
        if (you->location->east)
            next = you->location->east;
        printf("You go east.\n");
        break;
    case SOUTH:
        if (you->location->south)
            next = you->location->south;
        printf("You go south.\n");
        break;
    case WEST:
        if (you->location->west)
            next = you->location->west;
        printf("You go west.\n");
        break;
    default:
        ;
...
void
Map_destroy(void *self) {
    Map            *map = self;
    assert(map);

    if (map->hall)
        Room_destroy(map->hall);
    if (map->throne)
        Room_destroy(map->throne);
    if (map->kitchen)
        Room_destroy(map->kitchen);
    if (map->arena)
        Room_destroy(map->arena);

    if (map->you)
        map->you->_(destroy) (map->you);
    if (map->monster)
